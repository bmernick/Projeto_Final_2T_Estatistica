...
# Separar a variável de resposta e variáveis preditoras
total_cup_points <- dados_tratados_4$total_cup_points
dados_para_pca <- dplyr::select(dados_tratados_4, -total_cup_points, -country_of_origin)

# Padronizar e aplicar o PCA
X <- scale(dados_para_pca)
pca <- prcomp(X)
...
dados_modelo <- cbind(dados_pca, paises = dados_tratados_4$country_of_origin, total_cup_points = dados_tratados_4$total_cup_points)
